...
bool add1(HashTable& table, string key, string elem) {
...
bool add2(HashTable& table, string key, string elem) {
    Node* newNode = new Node;
    newNode->key = key;
    newNode->value = elem;
    newNode->next = nullptr;
    ...
    int counter = 0;
    while (table.table[hash] != nullptr && counter++ < M)
    {
        hash++;
    }
...
int main() {
    ...
    for (int i = 0; i < M; i++) {
        ...
        string newHuman = generateFullName() + " | " + generatePassportNum();
        add2(myTable, number, newHuman);
    }
